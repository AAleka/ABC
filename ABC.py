...
def artificial_bee_colony_optimization(target_function, food_sources=3, iterations=50, min_values=[-5, -5],
    ...
    sources = initial_sources(target_function, food_sources=food_sources, min_values=min_values, max_values=max_values)
    fitness = fitness_function(sources)
    while count <= iterations:
        if count > 0:
            print("Iteration = ", count, " f(x) = ", best_value, "x1 =", round(value[0], 4), "x2 =", round(value[1], 4))
        e_bee = employed_bee(target_function, sources, min_values=min_values, max_values=max_values)
        for i in range(0, employed_bees - 1):
            e_bee = employed_bee(target_function, e_bee[0], min_values=min_values, max_values=max_values)
        fitness = fitness_function(e_bee[0])
        o_bee = outlooker_bee(target_function, e_bee[0], fitness, e_bee[1], min_values=min_values,
                              max_values=max_values)
        for i in range(0, outlookers_bees - 1):
            o_bee = outlooker_bee(target_function, o_bee[0], fitness, o_bee[1], min_values=min_values,
                                  max_values=max_values)
        value = np.copy(o_bee[0][o_bee[0][:, -1].argsort()][0, :])
        if best_value > value[-1]:
            best_solution = np.copy(value)
            best_value = np.copy(value[-1])
        sources = scouter_bee(target_function, o_bee[0], o_bee[1], limit=limit)
        fitness = fitness_function(sources)
        count = count + 1
